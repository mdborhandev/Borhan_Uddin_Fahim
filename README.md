# Mohammed Borhan Uddin Fahim

Backend-focused Software Engineer with production-level SaaS experience.

## About Me

Software Engineer | ASP.NET Core Developer building scalable enterprise solutions with clean architecture, secure APIs, and optimized database performance.

**Location:** Chattogram, Bangladesh  
**Email:** mdborhan.dev@gmail.com  
**Phone:** 01906750597  
**LinkedIn:** linkedin.com/in/borhan-uddin-fahim

## Skills

- ASP.NET Core MVC & Web API (90%)
- Entity Framework Core (85%)
- PostgreSQL & SQL Server (80%)
- RESTful API & JWT (85%)
- HTML5 & CSS3 (90%)
- JavaScript & jQuery (80%)
- Bootstrap & Tailwind (85%)
- Git & GitHub (85%)

## Experience

### Junior Programmer
**Genuine Technology & Research Ltd.** | March 2024 - Present

- Developed SaaS-based HRM system for 200+ users
- Built RESTful APIs with JWT authentication
- Implemented RBAC and approval workflows
- Optimized API response by 30%

### Guest Trainer
**A K Khan UCEP TVET Institute** | Dec 2023 - Feb 2024

- Trained 40+ students in Frontend Dev
- HTML, CSS, JavaScript training

## Education

### Diploma in Computer Science
**A K Khan UCEP Polytechnic Institute** | 2020 - 2024

- GPA: 3.82/4.0

### Secondary School Certificate
**West Kadhurkhil School & College** | 2020

- GPA: 3.94/5.0

## Projects

### ERP - Enterprise Resource Planning
Comprehensive enterprise ERP solution for business process management including inventory, supply chain, HR, finance, and operational workflows.

**Tech:** ASP.NET Core MVC | Web API | EF Core | PostgreSQL

### Atrai - Accounting Software
Full-featured accounting software for financial management, invoicing, expense tracking, ledger management, and financial reporting.

**Tech:** ASP.NET Core MVC | Web API | EF Core | PostgreSQL

### Halda - SaaS HRM Platform
Employee management, payroll, attendance & performance modules with RBAC.

**Tech:** ASP.NET Core MVC | Web API | EF Core | PostgreSQL

### OKR - Goal & Task Management
OKR-based team goal tracking with performance dashboards.

**Tech:** ASP.NET Core MVC | EF Core | PostgreSQL

### SmartSLead - Lead Management
Comprehensive lead management for sales pipeline.

**Tech:** ASP.NET Core MVC | EF Core | PostgreSQL

### Gym Management
Comprehensive gym management system for member tracking, attendance, membership plans, and workout scheduling.

**Tech:** ASP.NET Core MVC | EF Core | PostgreSQL

## Stats

- 200+ Active Users
- 6 Major Projects
- 2+ Years Experience
- 40+ Students Mentored

## Connect

- [LinkedIn](https://www.linkedin.com/in/borhan-uddin-fahim/)
- [GitHub](https://github.com/mdborhandev)
- [Email](mailto:mdborhan.dev@gmail.com)

---

&copy; 2025 Mohammed Borhan Uddin Fahim. All Rights Reserved.